<template>
  ...
  <div class="col-4" style="border-right: 1px solid grey">
        ...
        <a class="btn text-right" @click="Type.Enabled = !Type.Enabled" style="display:block">
            {{ Type.Name }}
    ...
    <h3 class="text-right">Columnas</h3>
      <template v-for="(Column, cidx) in Columns" :key="cidx" >
        <a class="btn text-right" @click="Column.Enabled = !Column.Enabled" style="display:block">
            {{ Column.Name }}
      ...
      <h3 class="text-right">Modelos <small @click="ModelosInvertir()">(invertir selección)</small></h3>
      <template v-for="(Model, midx) in Models" :key="midx" >
        <a class="btn text-right" @click="Model.Enabled = !Model.Enabled" style="display:block">
            {{ Model.Name }}
    ...
    <div class="col-8">
        ...
        <table class="table table-hover">
            <thead>
                <th v-for="(Column, cidx) in Cols" :key="cidx" @click="ToggleSort(Column.Name)">
                    {{ Column.Name }} 
                    <span v-if="SortBy == Column.Name && SortDirection == 'DSC'"> 🔽</span>
                    <span v-if="SortBy == Column.Name && SortDirection == 'ASC'"> 🔼</span>
                </th>
            </thead>
            <tbody>
