  ...
  <div class="container-fluid">
    ...
    <div class="row" v-show="Tabs[0].Visible">
      <div class="col-2">
        ...
        <template v-if="Ok">
          <h3>Valores M:</h3>
          <ul>
            <li v-for="(item, idx) in KeyedDataM" :key="idx">
              {{ item.Key }} : {{ item.Value }}
            </li>
          </ul>
          ...
          <h3>Valores F:</h3>
          <ul>
            <li v-for="(item, idx) in KeyedDataF" :key="idx">
              {{ item.Key }} : {{ item.Value }}
            </li>
          </ul>
          ...
          <ul>
            <li v-for="(item, idx) in KeyedDiff" :key="idx">
              <span
                :style="{
                  color: KeyedDiffColors.find((x) => x.Key === item.Key).Value,
                }"
                >{{ item.Key }}</span
              >
              {{ item.Value }}
    ...
    <div class="row" v-show="Tabs[1].Visible" v-if="!RedrawingTable">
          ...
          <div class="col-md-3">
            <div v-if="TableShow === 'Experimental'">
              ...
              <input
                type="number"
                step="0.1"
                v-model="ExperimentalCorrection"
              />
            ...
            <tr v-for="(model, mIdx) in VisibleModels" :key="mIdx">
              ...
              <template v-for="(col, colIdx) in VisibleColumns" :key="colIdx">
                ...
                <td
                  ...
                  :style="{
                    'background-color':
                      GetValueMale(model, col) - GetValueFemale(model, col) > 0
                        ? 'aliceblue'
                        : 'lightpink',
                  }"
                >
                  {{
                    Cut(GetValueMale(model, col) - GetValueFemale(model, col))
                  }}
                </td>
                <td
                  ...
                  :style="
                    StyleHeatmap(
                      col,
                      GetValueMale(model, col) - GetValueFemale(model, col)
                    )
                  "
                  class="br1"
                >
      ...
      <div class="col-md-4">
        ...
        ><br />
        <span @click="ShowLatexCount = !ShowLatexCount"
          >ShowLatexCount {{ ShowLatexCount }}</span
        ><br />
        <span @click="ShowMinMax = !ShowMinMax"
          >ShowMinMax {{ ShowMinMax }}</span
        ><br />
        <span @click="ChangeHeatmapType"
          >UseSymmetricHeatMap {{ UseSymmetricHeatMap }}</span
        ><br />
        ...
        <span
          ...
          :style="{
            'background-color': ColumnMeta[col]?.Hide
              ? 'lightgrey'
              : 'lightblue',
          }"
        ...
        <span
          ...
          :style="{
            'background-color': ModelMeta[model]?.Hide
              ? 'lightgrey'
              : 'lightblue',
          }"
          style="margin-right: 10px"
...
export default {
  ...
  components: {
    ...
    QtyLineChart,
    TablaAdjetivos,
    ExploradorPredicciones,
  },
  created() {
    this.Columns.forEach((col) => (this.ColumnMeta[col] = { Hide: true }));
    this.Models.forEach(
      (col, idx) => (this.ModelMeta[col] = { Hide: false, Index: idx })
    );
  ...
  methods: {
    ...
    KVData(idx) {
      ...
      this.Labels.forEach((label) =>
        values.push({ Key: label, Value: modelm[label][this.ColumnKey] })
      );
      return values;
    },
    SelectTab(label) {
    ...
    ToggleAllModels(hidden) {
      Object.keys(this.ModelMeta).forEach(
        (meta) => (this.ModelMeta[meta].Hide = hidden)
      );
    },
    Cut(item) {
      item = item + "";
      ...
      if (exponente !== "") res = res + "e" + exponente;
      return res;
    },
    AsLatex() {},
    StyleHeatmap(column, val) {
      ...
      let color = "black";
      if (val > 0 && colorValue < 150) color = "white";
    ...
    HeatMap(column, val) {
      let normVal = this.Normalize(
        val,
        this.TableMin[column],
        this.TableMax[column]
      );
      let h = (1.0 - normVal) * 240;
      return "hsl(" + h + ", 100%, 50%)";
    },
    CustomHeatMap(column, val) {
      return this.CommonHeatMap(
        val,
        this.TableMin[column],
        this.TableMax[column]
      );
  ...
  computed: {
    ...
    DataTable() {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        tableM.push(
          ...Object.values(m).filter((xrow) => xrow.cat === this.CurrentLabel)
        );
        tableF.push(
    ...
    KeyedDiff() {
      ...
      for (let item of items) {
        arr.push({
          Key: item.Key,
          Value:
            this.KeyedDataM.find((x) => x.Key === item.Key).Value -
            this.KeyedDataF.find((x) => x.Key === item.Key).Value,
        });
      }
      return arr;
    },
    ...
    Labels() {
      let labels = Object.keys(this.Data[0]["py/tuple"][1]).filter(
        (x) => x !== "[_TOTAL]"
      );
      if (this.RemoveQuestionMarks) return labels.filter((x) => x !== "[???]");
      return labels;
    },
    Models() {
    ...
    TableMax() {
      ...
      t.VisibleColumns.forEach((column) => {
        let values = this.VisibleModels.map(
          (model) =>
            this.GetValueMale(model, column) -
            this.GetValueFemale(model, column)
        );
